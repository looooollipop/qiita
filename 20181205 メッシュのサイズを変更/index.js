...
function init(){
    ...
    //×1.0ボタンクリックでのイベント:メッシュのサイズを1.0倍にする
    document.getElementById("btn-1.0").addEventListener("click",function(){
        mesh.scale.set(1.0,1.0,1.0); //(x方向への倍率,y方向への倍率,z方向への倍率)
    });

    //×2.0ボタンクリックでのイベント:メッシュのサイズを1.0倍にする
    document.getElementById("btn-2.0").addEventListener("click",function(){
        mesh.scale.set(2.0,2.0,2.0); //(x方向への倍率,y方向への倍率,z方向への倍率)
    });

    //×3.0ボタンクリックでのイベント:メッシュのサイズを1.0倍にする
    document.getElementById("btn-3.0").addEventListener("click",function(){
        mesh.scale.set(3.0,3.0,3.0); //(x方向への倍率,y方向への倍率,z方向への倍率)
    });
}